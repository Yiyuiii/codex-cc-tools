...
DeepSeek requires `DEEPSEEK_API_KEY` or `OPENAI_API_KEY_DEEPSEEK` in the environment that starts Codex or the MCP server. `ark_coding_plan` requires `ARK_API_KEY` or `VOLCENGINE_API_KEY` when explicitly selected. `ark_agent_plan` requires `OPENAI_API_KEY_DOUBAO` when explicitly selected. Direct Gemini review requires `GEMINI_API_KEY`, `GOOGLE_API_KEY`, or `GOOGLE_GENERATIVE_AI_API_KEY` when explicitly selected, and can use request-level `geminiProxyUrl` before falling back to `HTTPS_PROXY` or `HTTP_PROXY` for transport. A variable exported only in a separate shell will not reach a GUI-launched Codex process. The default `anthropic` profile does not require `ANTHROPIC_AUTH_TOKEN` or `ANTHROPIC_API_KEY` when native Claude Code auth works.
