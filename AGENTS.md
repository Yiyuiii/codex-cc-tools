...
- Provider profiles describe which backend drives a task: Claude Code-backed profiles (`anthropic`, `deepseek`, `ark_coding_plan`, `ark_agent_plan`) or direct review-only profiles such as `gemini`.
...
- `docs/security.md` records provider environment inheritance, DeepSeek / Ark Coding Plan / Ark Agent Plan child-process routing, Gemini direct review routing, and provider-token redaction boundaries.
...
- 2026-06-17 provider update: `ark_coding_plan` is implemented as an explicit experimental provider profile. It reads `ARK_API_KEY` first, then `VOLCENGINE_API_KEY`, injects the Ark Coding Plan Anthropic-compatible Claude Code endpoint `https://ark.cn-beijing.volces.com/api/coding`, maps `opus` / `sonnet` / `haiku` to `ark-code-latest`, keeps legacy `glm-latest` / `Doubao-Seed-2.0-pro` / `doubao-seed-2.0-pro` inputs as compatibility aliases to `ark-code-latest`, and leaves `cc_delegate` defaulting to `deepseek`. The OpenAI-compatible endpoint `https://ark.cn-beijing.volces.com/api/coding/v3` is documented as not being the default for this Claude Code route.
- 2026-06-18 provider update: `ark_agent_plan` is implemented as an explicit experimental provider profile for the same Volcengine/Ark family through a separate plan/quota pool. It reads `OPENAI_API_KEY_DOUBAO`, injects the Ark Agent Plan Anthropic-compatible Claude Code endpoint `https://ark.cn-beijing.volces.com/api/plan`, maps `opus` / `sonnet` / `haiku` to `glm-5.2`, passes other model names through, and leaves `cc_delegate` defaulting to `deepseek`. The OpenAI-compatible endpoint `https://ark.cn-beijing.volces.com/api/plan/v3` is documented as not being the default for this Claude Code route.
...
- 2026-06-04 Gemini provider update: `gemini` is implemented as an explicit experimental direct `cc_review` provider profile. It reads `GEMINI_API_KEY` first, then `GOOGLE_API_KEY`, then `GOOGLE_GENERATIVE_AI_API_KEY`; calls Google Gemini `generateContent` at `https://generativelanguage.googleapis.com/v1beta`; maps `opus` / `sonnet` / `haiku` and `models/gemini-3.5-flash` to `gemini-3.5-flash`; supports request-level `geminiProxyUrl` first and inherited `HTTPS_PROXY` / `HTTP_PROXY` as fallbacks for transport; sends the key through `x-goog-api-key`; and rejects `cc_delegate` because Gemini direct API does not expose Claude Code filesystem, shell, or edit tools.
